...
else
{
 ...
 $queryStr = "Select * 
                from 
                    (
                        Select s.Student_ID, s.lastName, s.MiddleName, s.FirstName,
                                lp.VisualAcuityTest, 
                                lp.HeadTotoeAssessment, 
                                lp.VitalSigns, 
                                lp.FamilyMedicalRecord, 
                                lp.MedicalRecord,
                                s.Course
                        from Student s
                                Left Outer Join 
                                lab_procedure as lp on s.Student_ID=lp.uid
                    ) as s
                where 	(VisualAcuityTest IS NULL OR VisualAcuityTest = 0) AND 
                        (HeadTotoeAssessment IS NULL OR HeadTotoeAssessment =0) AND
                        (VitalSigns IS NULL OR VitalSigns=0) AND 
                        (FamilyMedicalRecord IS NULL OR FamilyMedicalRecord=0) AND 
                        (MedicalRecord IS NULL OR MedicalRecord = 0)";
  ...
  echo "
              ...
              <table align='center' cellspacing='4' cellpadding='5'>
				...
				<th>Middle Name</th>
                                ...
                                <th>Head To toe Assessment</th>
                                <th>Family Medical Record</th>
    ...
    while($recordSet = mysql_fetch_assoc($result))
               ...
               echo"	<tr>
                  ...
                  <td>$result[VisualAcuityTest]</td>
                  <td>$result[HeadTotoeAssessment]</td>
                  <td>$result[VitalSigns]</td>
                  <td>$result[FamilyMedicalRecord]</td>
                  <td>$result[MedicalRecord]</td>
                  <td>$result[Course]</td>
